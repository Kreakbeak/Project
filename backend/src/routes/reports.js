...
router.post('/refer-pest', protect, authorize('admin', 'agronomist'), referPestToReport);

// ID-based routes (these should come after specific routes)
router.get('/:id', protect, getReportById);
router.put('/:id', protect, authorize('admin', 'agronomist'), updateReportStatus);
router.delete('/:id', protect, authorize('farmer'), deleteReport);

// Generic routes (these should come last)
router.get('/', protect, authorize('admin', 'agronomist'), getAllReports);
